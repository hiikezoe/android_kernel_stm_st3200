...
void stm_smp_notify_cpu_stop(unsigned int cpu)
{
#if defined(CONFIG_KEXEC) && defined(CONFIG_SMP)

	static int code_copied;
	typedef void (*phys_reset_t)(unsigned long);
	phys_reset_t phys_reset;
	void *abap_prg_base = (void __iomem *)IO_ADDRESS(ABAP_PRG_BASE);
	/* copy abap secondary cpu startup code to abap programmable area */
	if (!code_copied++) {
		memcpy(abap_prg_base,
			stm_abap_sec_start, stm_abap_sec_start_size);
		__raw_writel(0, abap_prg_base + ABAP_JUMP_ADDR);
	}
...
#if HK_DEBUG
	{
	  void *abap_base = (void __iomem *)IO_ADDRESS(ABAP_PRG_BASE);
	  unsigned long	n = __raw_readl(abap_base + ABAP_JUMP_ADDR);
	  printk("boot_secondary: ABAP_JUMP_ADDR = 0x%08x\n, pen_release = %d\n", n, pen_release);
	}
#endif
...
void __init smp_init_cpus(void)
{
	...
	set_smp_notify_cpu_stop(stm_smp_notify_cpu_stop);
}
