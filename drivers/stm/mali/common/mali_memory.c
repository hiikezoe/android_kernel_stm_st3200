...
_mali_osk_errcode_t mali_memory_core_resource_os_memory(_mali_osk_resource_t * resource)
{
	...
	MALI_PRINT(("OS_MEMORY %uMB @ 0x%X : %s (order %u)\n",
			resource->size / (1024*1024),
			resource->base,
			resource->description,
			resource->alloc_order));
...
_mali_osk_errcode_t mali_memory_core_resource_dedicated_memory(_mali_osk_resource_t * resource)
{
	...
	/* do nothing for MEMORY regions of zero size (see Mali config.h) */
	if (0 == resource->size)
		MALI_SUCCESS;

	/* Request ownership of the memory */
	MALI_PRINT(("MEMORY %uMB @ 0x%X : %s (order %u)\n",
			resource->size / (1024*1024),
			resource->base,
			resource->description,
			resource->alloc_order));
