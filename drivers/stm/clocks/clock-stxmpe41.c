		...
		clk_t *_pix_main_clk, clk_t *_pix_aux_clk)
{
	...
	ret |= clk_register_table(&clk_clocks[CLKM_E_REF],
		CLKM_DDR_REF - CLKM_E_REF, 0);

	ret |= clk_register_table(&clk_clocks[CLKM_DDR_REF],
		CLKM_GPU_REF - CLKM_DDR_REF, 1);

	ret |= clk_register_table(&clk_clocks[CLKM_GPU_REF],
		ARRAY_SIZE(clk_clocks) - CLKM_GPU_REF, 0);
#endif
...
static int clkgenddr_recalc(clk_t *clk_p)
{
	...
	case CLKM_DDR_REF:
		clk_p->rate = clk_p->parent->rate;
		break;
	case CLKM_DDR_IC_LMI0 ...CLKM_DDR_IC_LMI1:
		{
		#if !defined(CLKLLA_NO_PLL)
		...
		break;
	case CLKM_DDR_DDR0 ... CLKM_DDR_DDR1:
