...
void __init stih415_configure_sbc(struct platform_device *sbc_config)
{
	/* gpio-power on board power supplies */
	struct stm_sbc_platform_data *data = dev_get_platdata(&sbc_config->dev);

	gpio_request(data->gpio, "POWER_PIO");
	gpio_direction_output(data->gpio, data->trigger_level);

	platform_device_register(sbc_config);
}
...
static struct platform_device stih415_sbc_device = {
	.name = "stm-rtc-sbc",
	.id = -1,
};
