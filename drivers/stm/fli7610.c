...
*/
static struct platform_device fli7610_pio_irqmux_devices[6] = {
	{
		/* PIO0-9 */
		...
		.num_resources = 2,
		.resource = (struct resource[]) {
			STM_PLAT_RESOURCE_MEM(0xfee3f080, 0x4),
			FLI7610_RESOURCE_IRQ(182),
		},
		.dev.platform_data = &(struct stm_plat_pio_irqmux_data) {
			.port_first = 17,
			.ports_num = 4,
		...
		.num_resources = 2,
		.resource = (struct resource[]) {
			STM_PLAT_RESOURCE_MEM(0xfee4f080, 0x4),
			FLI7610_RESOURCE_IRQ(183),
		},
		.dev.platform_data = &(struct stm_plat_pio_irqmux_data) {
			.port_first = 21,
			.ports_num = 5,
...
static int __init fli7610_postcore_setup(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(fli7610_pio_irqmux_devices); i++)
		platform_device_register(&fli7610_pio_irqmux_devices[i]);
